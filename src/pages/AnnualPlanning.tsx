...
const [editingActivityId, setEditingActivityId] = useState<string | null>(null);
  ...
  return (
    <div className={`p-8 max-w-5xl mx-auto space-y-12 ${isPrintingRange ? 'print:p-0 print:space-y-0 print:max-w-none print:m-0' : ''}`}>
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col print:p-0 print:border-none print:shadow-none">
        <div className="flex items-center justify-between mb-6 print:hidden">
          <div>
            <h2 className="text-2xl font-bold text-slate-800 tracking-tight">Planning Annuel</h2>
                    ...
                    return (
                      <div key={course.id} className={`flex items-center ${isPrintingRange && (printFit === 'contain' || printFit === 'height') ? 'flex-1 min-h-0' : 'min-h-[32px]'}`}>
                        ...
                        <div className="flex-1 flex relative h-full min-h-[16px] bg-slate-50 print:bg-white rounded-md border border-slate-100 overflow-hidden">
                          ...
                          {/* Draw Absences Background (Hatched) */}
                          {absences.filter(a => a.classId === c.id).map(a => {
                              const calWeeks = getWeekNumbers(a.startWeek, a.endWeek);
                              ...
                              return <div key={a.id} className="absolute top-0 bottom-0 mix-blend-multiply z-10" style={{ left: `${left}%`, width: `${width}%`, backgroundImage: 'repeating-linear-gradient(45deg, transparent, transparent 4px, rgba(220, 38, 38, 0.1) 4px, rgba(220, 38, 38, 0.1) 8px)' }} title={a.reason} />
                          })}
                          
                          {/* Draw Internships Background (Hatched) */}
                          {c.internships?.map(i => {
                              const hStartIdx = i.startWeek - 1;
                              const hEndIdx = i.endWeek - 1;
                              if (hStartIdx < 0 || hEndIdx < 0) return null;
                              ...
                              const left = (renderStart / displayedTotalWks) * 100;
                              const width = ((renderEnd - renderStart + 1) / displayedTotalWks) * 100;
                              return <div key={i.id} className="absolute top-0 bottom-0 mix-blend-multiply z-10" style={{ left: `${left}%`, width: `${width}%`, backgroundImage: 'repeating-linear-gradient(-45deg, transparent, transparent 4px, rgba(59, 130, 246, 0.1) 4px, rgba(59, 130, 246, 0.1) 8px)' }} title="Stage" />
                          })}
